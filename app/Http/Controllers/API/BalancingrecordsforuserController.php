...
class BalancingrecordsforuserController extends Controller
{
 ...
 //  $reptov = $request['actionaidsalesreportbydate'];
  ...
  $myrecordexists = \DB::table('monthlyreporttoviews')->where('ucret', '=', $userid)->count();
   ...
   //   'reporttype' => $request['reporttype'],
      ...
      'ucret' => $userid,
       ...
       if($myrecordexists > 0)
       {
        ...
        {
        $res = \DB::table('monthlyreporttoviews')->where('ucret', $userid)->update(['branchname' =>  $branch]);
      }
      $countryselection = strlen($country);
      if($countryselection > 0)
      {
      $res = \DB::table('monthlyreporttoviews')->where('ucret', $userid)->update(['countryname' =>  $country]);
      }
      
      $companyselection = strlen($company);
      if($companyselection > 0)
      {
      $res = \DB::table('monthlyreporttoviews')->where('ucret', $userid)->update(['companyname' =>  $company]);
      }
