...
use App\Companybranchinaction;
class CompanyuserregController extends Controller
{
...
{ 
  ...
  //  'branch', 'ucret','startdate','enddate',
    ...
    public function store(Request $request)
    {
        ...
        $existanceinthetable = \DB::table('companybranchinactions')->where('ucret', '=', $userid)->count();
        if($existanceinthetable < 1 )
        { Companybranchinaction::Create([
          //  'branch', 'ucret','startdate','enddate',
            //    'productcode' => $request['productcode'],
...
{ 
    ...
    $upp3 = $request['companyname'];
    $companyname = strlen($upp3);

    if($companyname > 0)

    {
         DB::table('companybranchinactions')
 ->where('ucret', $userid)
 ->update(['company' => $request['companyname']
