...
class ProductpurchaseDeleteconfirmedController extends Controller
{
...
//// Updating 
    ...
    public function destroy($id)
    {
     ...
     $suppliername =\DB::table('purchases')->where('id', '=', $id)->value('suppliername');
     $vattotal =\DB::table('purchases')->where('id', '=', $id)->value('vattotal');
     $tendecostproduct =\DB::table('purchases')->where('id', '=', $id)->value('grandtotal');
    //////////
    $qtydeli =\DB::table('purchases')->where('id', '=', $id)->value('qtydelivered');
    $linecostdelivered =\DB::table('purchases')->where('id', '=', $id)->value('grandtotal');
    $totaltaxdelivered =\DB::table('purchases')->where('id', '=', $id)->value('totaltaxdelivered');
    

/// Working on the product quantities
$currentproductquantity =\DB::table('products')->where('id', '=', $productid)->value('qty');
    $newproductqty = $currentproductquantity-$qtydeli;
    DB::table('products')
    ->where('id', $productid)
    ->update(array(
        'qty' => $newproductqty
...
///////////////////
//working on the vatin
$currentvatin =\DB::table('expensewalets')->where('id', '=', 5)->value('bal');
    $newvatin = $currentvatin-$totaltaxdelivered;
...
//// working on the supplier statement
$customertype =\DB::table('customers')->where('id', '=', $suppliername)->value('customertype');
$currentcustomerbalance =\DB::table('customers')->where('id', '=', $suppliername)->value('bal');
//supplier = 2
if($customertype == '2' && $currentcustomerbalance > 0)
{
  $newbal = $currentcustomerbalance-($linecostdelivered);

}
if($customertype == '2' && $currentcustomerbalance < 1)
{
  $newbal = $currentcustomerbalance+($linecostdelivered);
...
Customerstatement::Create([
  ...
  'description' => 'Alterattion: Removed item purchased',
  'openningbal' => $currentcustomerbalance,
  'amount' =>  ($linecostdelivered),
  'transactionmode'=> 2,
  'debitamount' =>0,
  'invoiceinaction' => $supplierinvoiceno,
  'resultatantbalance' => $newbal,
  'acttrans' => 4,
  'ucret' => $userid,

]);
DB::table('customers')->where('id', $suppliername)->update(array('bal' => $newbal));
...
$newordercostwithoutvat = \DB::table('purchases')->where('supplierinvoiceno', '=', $supplierinvoiceno)->sum('linetotal');
$totalinvoicewithvatnew = \DB::table('purchases')->where('supplierinvoiceno', '=', $supplierinvoiceno)->sum('totalinvoicewithvat');


$totaldeliveriwithoutvat = \DB::table('purchases')->where('supplierinvoiceno', '=', $supplierinvoiceno)->sum('lcostwithoutvat');
$totaltaxdelivered = \DB::table('purchases')->where('supplierinvoiceno', '=', $supplierinvoiceno)->sum('totaltaxdelivered');
$grandtotaldeliveredwithvat = \DB::table('purchases')->where('supplierinvoiceno', '=', $supplierinvoiceno)->sum('totalcostdeliverywithtax');
...
->update(array(
    ...
    'deliverycostwithoutvat' =>$totaldeliveriwithoutvat,
    'finalcost' =>$totaldeliveriwithoutvat,
    'totalvat' =>$totaltaxdelivered,
