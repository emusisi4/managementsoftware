...
<tr>
          ...
          <tr v-for="prodcates in cashintransfersshoprecords.data" :key="prodcates.id">
...
<td>
   ...
   <div  v-if="((prodcates.status)) == 1">
     ...
     <button v-show="prodcates.status == 0" type="button"   class="btn bg-info btn-xs waves-effect"
       @click="confirmCashinbranch(prodcates.id)"> Confirm Cash In </button>      
...
confirmCashinbranch(id){
   Swal.fire({
  title: 'Cash-IN Transfer',
...
if (result.isConfirmed) {
  this.form.delete('api/approvecashinsent/'+id).then(()=>{
       ...
       axios.get("api/cashintransfersshoprecords").then(({ data }) => (this.cashintransfersshoprecords = data));          
