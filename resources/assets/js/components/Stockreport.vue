...
<div v-if="generalProductscomponentAccess > 0 " > 
  ...
  <div  class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    <div class="info-box bg-indigo">
                        ...
                        <div class="content">
                            <div class="text"><b>STOCK VALUE (Last updated was done at {{lastipdatetimeanddate}}  )  </b>     
                            
<button type="button" class="btn  bg-brown btn-xs float-right" @click="confirmupdatestockvalue" >Click to update stock value  </button></div>
                            <div class="number count-to" data-from="0" data-to="257" data-speed="1000" data-fresh-interval="20">
                              <b>{{currencydetails}}  {{formatPrice(currentstockvalues)}}</b></div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="body">
                            ...
                            <ul class="nav nav-tabs" role="tablist">
                                ...
                                <li role="presentation" class="active" v-if="categoriesComponentaccess > 0">
                                    <a href="#home_with_icon_title" @click="laodProductdetails()" data-toggle="tab">
                                        <i class="material-icons"></i> Products stock Report
                                    </a>
                                </li>
         ...
         -->
            ...
            <th colspan="1"  style="font-size: 18px; text-align:center;    
             ...
             <th colspan="1"  style="font-size: 18px;   text-align:center;  border-bottom: 4px solid rgb(124 102 102); 
                 background-color: rgb(29 31 34 / 37%); color: #131378;"> Quantity Available</th>
               ...
               <th colspan="1"  style="font-size: 18px; text-align:center;    
               border-bottom: 4px solid rgb(124 102 102);     background-color: rgb(29 31 34 / 37%); color: #131378;">  CURRENT STOCK VALUE</th>
              ...
              <template v-if="prodcates.unit_measure">	{{prodcates.unit_measure.unitname}}</template></td>  
                ...
                <td><div class="musisialign">{{(prodcates.qty)}} </div></td>
                   ...
                   <td><div class="musisialignright">  {{formatPrice(prodcates.unitcost)}} </div></td>
                     <td><div class="musisialignright">  {{formatPrice(prodcates.unitcost*prodcates.qty)}} </div></td>
...
<th style="font-size: 18px; text-align:center;    
               border-top: 4px solid rgb(124 102 102);    
                background-color: rgb(211 211 211); color: #131378;" >
<div class="musisialignright"> Total stock Value : {{currencydetails}}  {{formatPrice(currentstockvalues)}}</div>
</th>
...
<!--  -->
  ...
  <div class="switch"  v-if="prodcates.discountstatus == '0' ">
        ...
        <button type="button"  class="btn bg-brown btn-xs waves-effect"  @click="productstockTransfer(prodcates)">Stock Transfer</button>
...
paginationroleAuthorisedcomponentsfeature(page = 1) {
     ...
     confirmupdatestockvalue(id){
   Swal.fire({
  title: 'STOCK VALUE UPDATE',
  text: "You are confirming that you want to check the stock value update at hand",
  icon: 'warning',
  showCancelButton: true,
  confirmButtonColor: '#3085d6',
  cancelButtonColor: '#d33',
...
/// send request ti
if (result.isConfirmed) {
   this.form.post('api/currentstockvalueupdate').then(()=>{

  
                        Swal.fire(
                          'Stock Value updated!',
                          'You have the current values now.',
                          'success'
                        )
                  
       axios.get("api/currentstockvalues").then(({ data }) => (this.currentstockvalues = data)); 
       axios.get("api/lastipdatetimeanddate").then(({ data }) => (this.lastipdatetimeanddate = data)); 
...
this.loadProductcategories();
 ...
 setInterval(() =>this.loadvatvalues(),2000);
        }
    }
</script>
